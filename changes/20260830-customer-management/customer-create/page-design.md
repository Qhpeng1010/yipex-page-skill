...
- Shell：yipex-default
- 信息密度：compact
- 原始需求：/yipex:page 生成客户管理流程，包含客户列表和新增客户两个关联页面。客户列表支持关键词、客户类型和状态筛选，表格顶部提供“新增客户”按钮；点击后进入新增客户页面。新增页面包含基础信息、联系人和结算信息，提交成功后返回客户列表，并用 message 提示成功，按创建时间倒序展示新客户；取消时直接返回且不保存
...
| 层级 | 操作 | 位置 | 结果 |
| --- | --- | --- | --- |
| 主操作 | 提交（`save-customer`） | 固定吸底栏最右侧 | 校验并保存客户，返回列表后置顶新客户并提示成功 |
